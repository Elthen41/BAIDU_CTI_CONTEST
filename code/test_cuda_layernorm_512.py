...
def check_correctness(ext, shape, dtype, eps=1e-5):
    ...
    expected = F.layer_norm(x, (512,), weight, bias, eps)
    actual = ext.layernorm_512(x, weight, bias, eps)

    rtol, atol = tolerances(dtype)
    torch.testing.assert_close(actual, expected, rtol=rtol, atol=atol)
    max_abs_err = (actual.float() - expected.float()).abs().max().item()
    print(f"[OK] correctness dtype={dtype}, shape={shape}, max_abs_err={max_abs_err:.3e}")
...
def check_error_paths(ext):
    ...
    try:
        ...
        print(f"[OK] width check raised: {str(exc).splitlines()[0]}")
    else:
        raise AssertionError("last dimension != 512 should raise RuntimeError")

    x = torch.randn((2, 512), device="cuda", dtype=torch.float32)
    try:
        ext.layernorm_512(x, weight.float(), bias.float(), 1e-5)
    except RuntimeError as exc:
...
def benchmark(ext, shape, dtype, eps=1e-5, warmup=50, iters=500):
    ...
    for _ in range(warmup):
        ext.layernorm_512(x, weight, bias, eps)
    ...
    start.record()
    for _ in range(iters):
        F.layer_norm(x, (512,), weight, bias, eps)
    end.record()
    torch.cuda.synchronize()
