...
using FragC_f = Vec<float, 4>;
using FragS_GROUP = Vec<half, 2>; // weight per-group quantization scales
...
__device__ inline void cp_async4_a100_optimized(void* smem_ptr, const void* glob_ptr) {
    const int BYTES = 16;
    uint32_t smem = static_cast<uint32_t>(__cvta_generic_to_shared(smem_ptr));
    ...
    asm volatile(
        "{\n"
        "   .reg .b64 policy;\n"
        "   createpolicy.fractional.L2::evict_normal.b64 policy, 0.75;\n"
        "   cp.async.cg.shared.global.L2::cache_hint [%0], [%1], %2, policy;\n"
        "}\n" :: "r"(smem), "l"(glob_ptr), "n"(BYTES)
    );
}
...
__device__ inline void cp_async2(void* smem_ptr, const void* glob_ptr) {
  ...
  *((unsigned short*)smem_ptr) = *((unsigned short*)glob_ptr);
}
...
>
__global__ void Marlin_w4a4_1(
  const int4* __restrict__ A, //  input matrix of shape mxk
  const int4* __restrict__ B, // 4bit quantized weight matrix of shape kxn
        float4* __restrict__ C, //  global_reduce buffer of shape (max_par*16*4)xn
        float4* __restrict__ D, //  output buffer of shape mxn
        half* __restrict__ s1, //  activation per-group quantization scales of shape
        float4* __restrict__ s2, //  weight per-group quantization scales of shape
  ...
  if (slice_col_par >= n_tiles) {
    ...
    s1 += (slice_col_par / n_tiles) * 16 * thread_m_blocks * prob_k / (group_blocks * 32);
  ...
  auto init_slice = [&] () {
    ...
    if (slice_col == n_tiles) {
      ...
      s1 += 16 * thread_m_blocks * prob_k / (group_blocks * 32);//
      locks += n_tiles;
      slice_col = 0;
    }
  ...
  int s1_gl_stride = prob_k/(group_blocks*32);
  constexpr int s1_sh_stride = 16 * thread_m_blocks;
  int s1_sh_stage = s1_sh_stride;
  int s1_gl_rd_delta = s1_gl_stride;
  int s2_gl_stride = prob_n / 8;
  constexpr int s2_sh_stride = 16 * thread_n_blocks / 8;
  int s2_sh_stage = s2_sh_stride;
  int s2_gl_rd_delta = s2_gl_stride;
//////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////
  int s1_gl_rd, s1_sh_wr, s1_sh_rd;
  bool s1_sh_wr_pred;
  int s1_sh_wr_iters = 0;
  if constexpr (group_blocks != -1) {
    if(group_blocks >= thread_k_blocks)
    {
      s1_gl_rd = threadIdx.x * s1_gl_stride;
      s1_gl_rd +=  ((thread_k_blocks * slice_row) / group_blocks) ;
      s1_sh_wr = threadIdx.x / 16 * 16 + (threadIdx.x % 16 % 8) * 2 + (threadIdx.x % 16 / 8);
      s1_sh_rd = (threadIdx.x % 32) / 4;
      s1_sh_wr_pred = threadIdx.x < s1_sh_stride;
    } else {
      s1_sh_wr_iters = ceildiv(thread_k_blocks, group_blocks);
      s1_sh_stage = s1_sh_wr_iters * s1_sh_stride;
      s1_gl_rd = threadIdx.x * s1_gl_stride;
      s1_gl_rd += slice_row * s1_sh_wr_iters;
      s1_sh_wr = threadIdx.x / 16 * 16 + (threadIdx.x % 16 % 8) * 2 + (threadIdx.x % 16 / 8);
      s1_sh_wr_pred = threadIdx.x < s1_sh_stride;
    }
  }
///////////////////////////////////////////////////////////////////////////////////////////////////////
  int s2_gl_rd, s2_sh_wr, s2_sh_rd;
  bool s2_sh_wr_pred;
  int s2_sh_wr_iters = 0;
  if constexpr (group_blocks != -1) {
    if(group_blocks >= thread_k_blocks)
    {
      s2_gl_rd = s2_gl_stride * ((thread_k_blocks * slice_row) / group_blocks) + s2_sh_stride * slice_col + threadIdx.x;
      s2_sh_wr = threadIdx.x;
      s2_sh_rd = 8 * ((threadIdx.x / 32) % (thread_n_blocks / 4)) + (threadIdx.x % 32) % 4;
      s2_sh_wr_pred = threadIdx.x < s2_sh_stride;
    } else {
      s2_sh_wr_iters = ceildiv(thread_k_blocks, group_blocks);
      s2_sh_stage = s2_sh_wr_iters * s2_sh_stride;
      s2_gl_rd = s2_gl_stride * slice_row * s2_sh_wr_iters + s2_sh_stride * slice_col + threadIdx.x;
      s2_sh_wr = threadIdx.x;
      s2_sh_wr_pred = threadIdx.x < s2_sh_stride;
    }
  }
...
//////////////////////////////////////////////////////////////////
  ...
  int4* sh_a = reinterpret_cast<int4*>(sh);
  int4* sh_b = sh_a + (stages * a_sh_stage);
  float4* sh_s1 = reinterpret_cast<float4*>(sh_b+ stages * b_sh_stage);
  float4* sh_s2 = sh_s1 + s1_sh_stage/8*stages;

  // Register storage for double buffer of shared memory reads.
  FragA frag_a[2][thread_m_blocks];
  FragB frag_b[2][4][2];
  FragC_f frag_c_f[thread_m_blocks][4][2];
  FragS_GROUP frag_s2[2][8];
  FragS_GROUP frag_s1[thread_m_blocks][2];

  // Zero accumulators.
  auto zero_accums = [&] () {
    #pragma unroll
    for (int i = 0; i < thread_m_blocks * 4 * 2 * 4; i++)
      reinterpret_cast<float*>(frag_c_f)[i] = 0;
  };
  ...
  auto fetch_to_shared = [&] (int pipe, int a_off, bool pred = true) {
    if (pred) {
      ...
      if constexpr (group_blocks != -1) {
        if(group_blocks >= thread_k_blocks){
          int count=group_blocks/8;
          int count1=2 - (group_blocks== 4);
          if (pipe % (count1*4 / thread_k_blocks) == 0) {
            float4* sh_s1_stage = sh_s1 + s1_sh_stage/8 * pipe;
            if (s1_sh_wr_pred)
              reinterpret_cast<half*>(sh_s1_stage)[s1_sh_wr] = s1[s1_gl_rd];
            if(count==0||count==1 || count==2&pipe==2 || count==4&slice_iters%8==5)
              s1_gl_rd += 1;
          }

        } else { // group_blocks < thread_k_blocks
          float4* sh_s1_stage = sh_s1 + s1_sh_stage/8 * pipe;
          for(int i = 0; i < s1_sh_wr_iters; ++i)
          {
            if(s1_sh_wr_pred)
            {
              reinterpret_cast<half*>(sh_s1_stage)[s1_sh_stride * i + s1_sh_wr] = s1[i + s1_gl_rd];
            }
          }
          s1_gl_rd += s1_sh_wr_iters;
        }
      }
      ...
      if constexpr (group_blocks != -1) {
        if(group_blocks >= thread_k_blocks){
          int count=group_blocks/8;
          int count1=2 - (group_blocks== 4); //4 8 16 32 -> 1 2 2 2

          if (pipe % (count1*4 / thread_k_blocks) == 0) {
            float4* sh_s2_stage = sh_s2 + s2_sh_stage * pipe;
            if (s2_sh_wr_pred)
              cp_async4_stream(&sh_s2_stage[s2_sh_wr], &s2[s2_gl_rd]);
            if(count==0||count==1 || count==2&pipe==2  || count==4&slice_iters%8==5)
              s2_gl_rd += s2_gl_rd_delta;
          }
        } else { // group_blocks < thread_k_blocks
          float4* sh_s2_stage = sh_s2 + s2_sh_stage * pipe;
          for(int i = 0; i < s2_sh_wr_iters; ++i)
          {
            if(s2_sh_wr_pred)
            {
              cp_async4_stream(&sh_s2_stage[s2_sh_stride * i + s2_sh_wr], &s2[s2_gl_stride * i + s2_gl_rd]);
            }
          }
          s2_gl_rd += s2_gl_rd_delta * s2_sh_wr_iters;
        }
      }
    }
  ...
  auto fetch_to_registers = [&] (int k, int pipe) {
    ...
    if constexpr (group_blocks != -1) {
      if(group_blocks >= thread_k_blocks){
        int count1=2 - (group_blocks== 4);
        float4* sh_s1_stage = sh_s1 + s1_sh_stage/8 * ((count1*4 / thread_k_blocks) * (pipe / (count1*4 / thread_k_blocks)));
        #pragma unroll
        for (int i = 0; i < thread_m_blocks; i++) {
          *reinterpret_cast<float*>(&frag_s1[i][k%2]) = reinterpret_cast<float*>(sh_s1_stage)[8 * i + s1_sh_rd];
        }

      } else { // group_blocks < thread_k_blocks
        int s1_sh_rd_row = (k % 2) * (thread_k_blocks / (b_sh_wr_iters/2)) + (threadIdx.x / 32) / (thread_n_blocks / 4);
        int s1_sh_rd_row_real = s1_sh_rd_row / group_blocks;
        s1_sh_rd = s1_sh_rd_row_real * s1_sh_stride/2 + (threadIdx.x % 32) / 4;;
        float4 *sh_s1_stage = sh_s1 + s1_sh_stage/8 * pipe;
        #pragma unroll
        for (int i = 0; i < thread_m_blocks; i++) {
          *reinterpret_cast<float*>(&frag_s1[i][k%2]) = reinterpret_cast<float*>(sh_s1_stage)[8 * i + s1_sh_rd];
    ...
    if constexpr (group_blocks != -1) {
      if(group_blocks >= thread_k_blocks){
        int count1=2 - (group_blocks== 4);
        float4* sh_s2_stage = sh_s2 + s2_sh_stage * ((count1*4 / thread_k_blocks) * (pipe / (count1*4 / thread_k_blocks)));
        reinterpret_cast<float4*>(&frag_s2[k % 2])[0] = sh_s2_stage[s2_sh_rd];
        reinterpret_cast<float4*>(&frag_s2[k % 2])[1] = sh_s2_stage[s2_sh_rd+4];

      } else { // group_blocks < thread_k_blocks
        int s2_sh_rd_row = (k % 2) * (thread_k_blocks / (b_sh_wr_iters/2)) + (threadIdx.x / 32) / (thread_n_blocks / 4);
        int s2_sh_rd_row_real = s2_sh_rd_row / group_blocks;
        s2_sh_rd = s2_sh_rd_row_real * s2_sh_stride + 8 * ((threadIdx.x / 32) % (thread_n_blocks / 4)) + (threadIdx.x % 32) % 4;
        float4 *sh_s2_stage = sh_s2 + s2_sh_stage * pipe;
        reinterpret_cast<float4*>(&frag_s2[k % 2])[0] = sh_s2_stage[s2_sh_rd];
        reinterpret_cast<float4*>(&frag_s2[k % 2])[1] = sh_s2_stage[s2_sh_rd+4];
      }
  ...
  auto matmul = [&] (int k) {
    ...
    auto write_1 = [&] (float& c0f,float& c1f, int c0, int c1, half a_s, FragS_GROUP& w_s) {
      c0f += int32_to_float(c0) * __half2float(w_s[0]) * __half2float(a_s);
      c1f += int32_to_float(c1) * __half2float(w_s[1]) * __half2float(a_s);
    };
    ...
    for (int j = 0; j < 4; j++) {
      ...
      #pragma unroll
      for (int i = 0; i < thread_m_blocks; i++) {
        mma(frag_a[k % 2][i], frag_b[k%2][j][0], frag_c[i][j][0]);
        mma(frag_a[k % 2][i], frag_b[k%2][j][1], frag_c[i][j][1]);

        write_1(frag_c_f[i][j][0][0], frag_c_f[i][j][0][1],frag_c[i][j][0][0], frag_c[i][j][0][1], frag_s1[i][k%2][0], frag_s2[k % 2][2 * j + 0]);
        write_1(frag_c_f[i][j][0][2], frag_c_f[i][j][0][3],frag_c[i][j][0][2], frag_c[i][j][0][3], frag_s1[i][k%2][1], frag_s2[k % 2][2 * j + 0]);
        write_1(frag_c_f[i][j][1][0], frag_c_f[i][j][1][1],frag_c[i][j][1][0], frag_c[i][j][1][1], frag_s1[i][k%2][0], frag_s2[k % 2][2 * j + 1]);
        write_1(frag_c_f[i][j][1][2], frag_c_f[i][j][1][3],frag_c[i][j][1][2], frag_c[i][j][1][3], frag_s1[i][k%2][1], frag_s2[k % 2][2 * j + 1]);
        ...
        for (int i = red_off; i > 0; i /= 2) {
          if (i <= red_idx && red_idx < 2 * i) {
            ...
            for (int j = 0; j < 4 * 2; j++) {
              ...
              sh[red_sh_wr] = reinterpret_cast<float4*>(&frag_c_f)[4 * 2 * m_block + j];
            }
          }
          __syncthreads();
        }
    ...
    if (threadIdx.x < active_threads) {
      ...
      for (int i = 0; i < thread_m_blocks * 4; i++) {
        if (i < (thread_m_blocks - 1) * 4 || 8 * (i / 2) + row < prob_m) {
          if (!first) {
            float4 d_red1 = sh[c_sh_wr + i * c_sh_wr_delta];
            float4 d_red2 = sh[c_sh_wr + i * c_sh_wr_delta + 1];
            #pragma unroll
            for (int j = 0; j < 4; j++) {
              reinterpret_cast<float*>(&frag_c_f)[4 * 2 * 4 * (i / 4) + 4 * j + (i % 4)] +=
                reinterpret_cast<float*>(&d_red1)[j];
            }
            #pragma unroll
            for (int j = 0; j < 4; j++) {
              reinterpret_cast<float*>(&frag_c_f)[4 * 2 * 4 * (i / 4) + 4 * (j + 4) + (i % 4)] +=
                reinterpret_cast<float*>(&d_red2)[j];
            }
          }
          if (!last) {
            float4 d1, d2;
            #pragma unroll
            for (int j = 0; j < 4; j++) {
              reinterpret_cast<float*>(&d1)[j] =
                reinterpret_cast<float*>(&frag_c_f)[4 * 2 * 4 * (i / 4) + 4 * j + (i % 4)];
            }
            #pragma unroll
            for (int j = 0; j < 4; j++) {
              reinterpret_cast<float*>(&d2)[j] =
                reinterpret_cast<float*>(&frag_c_f)[4 * 2 * 4 * (i / 4) + 4 * (j + 4) + (i % 4)];
            }
  ...
  auto write_result = [&] () {
    ...
    if (threadIdx.x / 32 < thread_n_blocks / 4) {
      ...
      for (int i = 0; i < thread_m_blocks; i++) {
        ...
        for (int j = 0; j < 4; j++) {
          int wr = d_sh_wr + 8 * j;
          write(wr + (4 * d_sh_stride) * 0 + 0, frag_c_f[i][j][0][0], frag_c_f[i][j][0][1]);
          write(wr + (4 * d_sh_stride) * 8 + 0, frag_c_f[i][j][0][2], frag_c_f[i][j][0][3]);
          write(wr + (4 * d_sh_stride) * 0 + 4, frag_c_f[i][j][1][0], frag_c_f[i][j][1][1]);
          write(wr + (4 * d_sh_stride) * 8 + 4, frag_c_f[i][j][1][2], frag_c_f[i][j][1][3]);
        }
...
#define CALL_IF(THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, GROUP_BLOCKS) \
    ...
    cudaFuncSetAttribute( \
      Marlin_w4a4_1<THREADS, THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, STAGES, GROUP_BLOCKS>, \
    ...
    ><<<blocks, THREADS, max_shared_mem, stream>>>( \
      A_ptr, B_ptr, C_ptr, D_ptr, s1_ptr, s2_ptr, \
...
int apex4_cuda_w4a4_group(
  ...
  float4* D_ptr = (float4*) D;
  half* s1_ptr = ( half*) s1;
  float4* s2_ptr = (float4*) s2;
  ...
  for (int i = 0; i < tot_m_blocks; i += 4) {
    ...
    s1_ptr += 16 * thread_m_blocks * (prob_k / groupsize) * par;
  }

  return ret;
}

void apex4_gemm_w4a4_group(
  ...
  int groupsize = (s2.numel() == 0) ? -1 : prob_k / s2.size(0);
  if (groupsize != -1 && groupsize * s2.size(0) != prob_k)
    AT_ERROR("k=", prob_k, " not compatible with ", s2.size(0), " groups.");
  if (workspace.numel() < prob_n / 128 * max_par)
    AT_ERROR("workspace must be of size at least ", prob_n / 128 * max_par, ".");
  if (s1.dtype() != torch::kFloat16)
     AT_ERROR("s1 dtype must be kFloat16, but got ", s1.dtype(), ".");
  if (s2.dtype() != torch::kFloat16)
     AT_ERROR("s2 dtype must be kFloat16, but got ", s2.dtype(), ".");
  int dev = A.get_device();
  int err = apex4_cuda_w4a4_group(
    A.data_ptr(),
    B.data_ptr(),
    C.data_ptr(),
    D.data_ptr(),
