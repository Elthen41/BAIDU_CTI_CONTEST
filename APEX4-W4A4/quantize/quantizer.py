...
from quantize.matmul_had import *  # noqa: F403, F401
from quantize.qparams import *  # noqa: F403, F401
from quantize.utils import min_max_quantize
...
def stochastic_round_ste(x: torch.Tensor, qmax: int, qmin: int) -> torch.Tensor:
    """Implement Straight-Through Estimator for stochastic rounding operation.
    ...
    Returns:
        Stochastically rounded tensor with gradient flow preserved
    """
    with torch.no_grad():
        floor_val = torch.floor(x)
        ceil_val = torch.ceil(x)
        prob = x - floor_val
        prob = torch.where(prob < 0.35, torch.zeros_like(prob), prob)
        prob = torch.where(prob > 0.65, torch.ones_like(prob), prob)

        qweight = torch.where(torch.rand_like(prob) < prob, ceil_val, floor_val)

    return (qweight.clamp(qmin, qmax) - x).detach() + x
...
class UniformActivationQuantizer(nn.Module):
    """Uniform quantization for activations with dynamic range calculation.
    
    Supports per-token and group-wise quantization strategies for efficient
    activation quantization in neural networks.
    
    Args:
        qparams: Quantization parameters containing bits, group_size, etc.
        symmetric: Whether to use symmetric quantization
        quant_range: Quantization granularity (per_tensor, per_token, per_channel, per_group)
    """
    ...
    def __init__(
        ...
        symmetric: bool = True,
        quant_range: str = "per_channel",
    ) -> None:
        super().__init__()

        valid_ranges = ["per_tensor", "per_token", "per_channel", "per_group"]
        if quant_range not in valid_ranges:
            msg = f"quant_range must be one of {valid_ranges}"
            raise ValueError(msg)
            
        self.qparams = qparams
        self.symmetric = symmetric
        self.n_bit = qparams.bits
        self.group_size = qparams.group_size
        self.bits_prop = [1.0]  # Only uniform precision supported
        self.quant_range = quant_range

        if not self.bits_prop:
            msg = "Only support uniform precision quantization for activation"
            raise AssertionError(msg)

        # Set quantization ranges based on symmetry
        if self.symmetric:
            self.quant_min = -(2 ** (self.n_bit[0] - 1))
            self.quant_max = 2 ** (self.n_bit[0] - 1) - 1
        else:
            self.quant_min = 0
            self.quant_max = 2 ** self.n_bit[0] - 1

    def dynamic_quant(self, x: torch.Tensor, fakequant: bool, return_raw: bool = False) -> torch.Tensor:
        """Dynamic quantization controlled by group_size configuration.
        
        Args:
            x: Input tensor to quantize
            fakequant: Whether to apply fake quantization (training mode)
        ...
        group_size = list(self.group_size.values())[0]
        
        if group_size == -1:
            # Per-token quantization: independent quantization per token
            quant_scale, quant_zeros = self._compute_per_token_params(x)
        elif group_size > 0:
            # Group-wise quantization: partition last dimension into groups
            quant_scale, quant_zeros = self._compute_group_wise_params(x, group_size)
            x = x.view(*x.shape[:-1], -1, group_size)
        else:
            msg = f"Invalid group_size: {group_size}. Must be -1 or positive integer."
            raise ValueError(msg)

        # Apply quantization
        x_quantized = self._apply_quantization(x, quant_scale, quant_zeros, fakequant)
        
        if not return_raw:
            return x_quantized.view(original_shape), quant_scale
        else:
            return x_quantized.div(quant_scale).view(original_shape), quant_scale

    def _compute_per_token_params(self, x: torch.Tensor) -> Tuple[torch.Tensor, Union[torch.Tensor, int]]:
        """Compute quantization parameters for per-token quantization."""
        if self.symmetric:
            quant_scale = x.abs().max(dim=-1, keepdim=True)[0] / self.quant_max
            quant_zeros = 0
        else:
            xmax = torch.amax(x, dim=-1, keepdim=True)
            xmin = torch.amin(x, dim=-1, keepdim=True)
            quant_scale = (xmax - xmin) / self.quant_max
            quant_zeros = -xmin / quant_scale
            quant_zeros = quant_zeros.round()
        
        return quant_scale, quant_zeros

    def _compute_group_wise_params(
        self, 
        x: torch.Tensor, 
        group_size: int
    ) -> Tuple[torch.Tensor, Union[torch.Tensor, int]]:
        """Compute quantization parameters for group-wise quantization."""
        last_dim = x.shape[-1]
        if last_dim % group_size != 0:
            msg = f"Last dimension {last_dim} must be divisible by group_size {group_size}"
            raise AssertionError(msg)
        ...
        if self.symmetric:
            quant_scale = x_grouped.abs().max(dim=-1, keepdim=True)[0] / self.quant_max
            quant_zeros = 0
        else:
            xmax = torch.amax(x_grouped, dim=-1, keepdim=True)
            xmin = torch.amin(x_grouped, dim=-1, keepdim=True)
            quant_scale = (xmax - xmin) / self.quant_max
            quant_zeros = -xmin / quant_scale
            quant_zeros = quant_zeros.round()
    ...
    def _apply_quantization(
        ...
        quant_zeros: Union[torch.Tensor, int],
        fakequant: bool,
    ) -> torch.Tensor:
        """Apply quantization with optional fake quantization for training."""
        quantized = (x / quant_scale).round() + quant_zeros
        quantized = quantized.clamp(self.quant_min, self.quant_max)
        dequantized = (quantized - quant_zeros) * quant_scale
        
        if fakequant:
            # Straight-through estimator for training
            return (dequantized - x).detach() + x
        
        return dequantized

    def forward(self, x: torch.Tensor, return_raw: bool = False) -> torch.Tensor:
        """Forward pass for activation quantization.
        ...
        shape = x.size()
        x = x.view(-1, shape[-1])
        y, quant_scale = self.dynamic_quant(x, fakequant=self.training, return_raw=return_raw)
        return y.reshape(shape), quant_scale


class UniformAffineQuantizer(nn.Module):
    """Optimized uniform affine quantizer for neural network weights.
    
    Provides efficient mixed-precision quantization with vectorized operations,
    learnable quantization parameters, and optional double quantization.
    Supports channel-wise quantization when group_size=-1.
    ...
    Args:
        ...
        qat: Enable quantization-aware training with double quantization
    ...
    def __init__(
        ...
        self.double_group_size = (
            qparams.scale_groups if qparams.scale_groups != -1 else shape[0]
        )
        self.double_quant_scale_bits = qparams.scale_bits
        self.double_quant_zero_bits = qparams.scale_bits
        ...
        # Validate group size divisibility (skip -1 for channel-wise quantization)
        if self.group_size is not None:
            valid_group_sizes = [gs for gs in self.group_size.values() if gs != -1]
            if valid_group_sizes:
                min_group_size = min(valid_group_sizes)
        ...
        self.qat = qat
        if self.qat:
            print(f"Enable double quantization for {name}")
    ...
    def _get_num_groups(self, group_size: int, num_cols: int, shape: Tuple[int, int]) -> int:
        """Calculate number of groups based on group_size, handling channel-wise case.
        
        Args:
            group_size: Group size (-1 for channel-wise, positive for group-wise)
            num_cols: Number of columns for this bit group
            shape: Overall tensor shape
            
        Returns:
            Number of groups
        """
        if group_size == -1:
            # Channel-wise quantization: one group per output channel (row)
            return 1
        else:
            # Group-wise quantization: divide columns into groups
            return num_cols // group_size
    ...
    def _initialize_learnable_parameters(self, shape: Tuple[int, int]) -> None:
        ...
        if self.shift:
            ...
            # Calculate total number of groups across all bit groups
            total_groups = 0
            for i in range(len(group_sizes)):
                num_cols = self._get_column_count(i)
                num_groups = self._get_num_groups(group_sizes[i], num_cols, shape)
                if group_sizes[i] == -1:
                    # Channel-wise: one parameter per output channel
                    total_groups += shape[0]
                else:
                    # Group-wise: shape[0] * num_groups parameters
                    total_groups += shape[0] * num_groups
            
            self.scale_dequant_post = torch.zeros(total_groups, 1)
            if not self.symmetric:
                self.zeros_dequant_post = torch.zeros(total_groups, 1)
            else:
    ...
    def _initialize_shift_parameters(
        ...
        self.scale_max_scale_2th = nn.ParameterList()
        self.scale_min_scale_2th = nn.ParameterList()
        self.zeros_max_scale_2th = nn.ParameterList()
        self.zeros_min_scale_2th = nn.ParameterList()
        ...
        for i in range(len(group_sizes)):
            ...
            if group_size == -1:
                # Channel-wise quantization: one parameter per output channel
                param_shape = (shape[0], 1)
                double_groups = shape[0] // self.double_group_size
                double_param_shape = (double_groups, 1)
            else:
                # Group-wise quantization
                num_groups = num_cols // group_size
                param_shape = (shape[0], num_groups)
                double_groups = shape[0] // self.double_group_size
                double_param_shape = (double_groups, num_groups)
            
            # Scale parameters
            scale_param = torch.randn(param_shape) * 1e-5
            self.scale_dequant_post.append(
                nn.Parameter(scale_param.reshape(-1, 1).half())
            )
            
            max_scale_param = torch.ones(param_shape) * 4
            self.max_scale.append(
                nn.Parameter(max_scale_param.reshape(-1, 1).half())
            )
            
            # Double quantization scale parameters
            scale_2th_param = torch.ones(double_param_shape) * 4
            self.scale_max_scale_2th.append(nn.Parameter(scale_2th_param.half()))
            self.scale_min_scale_2th.append(nn.Parameter(scale_2th_param.half()))

            # Zero-point parameters (asymmetric only)
            if not self.symmetric:
                zeros_param = torch.randn(param_shape) * 1e-5
                self.zeros_dequant_post.append(
                    nn.Parameter(zeros_param.reshape(-1, 1).half())
                )
                
                min_scale_param = torch.ones(param_shape) * 4
                self.min_scale.append(
                    nn.Parameter(min_scale_param.reshape(-1, 1).half())
                )
                
                self.zeros_max_scale_2th.append(nn.Parameter(scale_2th_param.half()))
                self.zeros_min_scale_2th.append(nn.Parameter(scale_2th_param.half()))
    ...
    def _compute_bits_per_weight(self) -> float:
        ...
        for i, end_idx in enumerate(self.bits_column_end_index):
            ...
            if group_size == -1:
                # Channel-wise: one scale per output channel
                num_scale_groups = 1
                scale_bits = self.shape[0] * (
                    self.double_quant_scale_bits if self.qat else 16
                )
            else:
                # Group-wise: multiple scales per channel
                num_scale_groups = num_cols // group_size
                scale_bits = num_scale_groups * (
                    self.double_quant_scale_bits if self.qat else 16
                )
            ...
            if not self.symmetric:
                if group_size == -1:
                    zero_bits = self.shape[0] * (
                        self.double_quant_zero_bits if self.qat else 16
                    )
                else:
                    zero_bits = num_scale_groups * (
                        self.double_quant_zero_bits if self.qat else 16
                    )
    ...
    def double_quant(
        self,
        scale_per_bits: torch.Tensor,
        zeros_per_bits: torch.Tensor,
        scale_max_scale: torch.Tensor,
        scale_min_scale: torch.Tensor,
        zeros_max_scale: torch.Tensor,
        zeros_min_scale: torch.Tensor,
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """Apply double quantization to scales and zero-points.
        
        Args:
            scale_per_bits: Quantization scales
            zeros_per_bits: Quantization zero-points
            scale_max_scale: Maximum scale for scale quantization
            scale_min_scale: Minimum scale for scale quantization
            zeros_max_scale: Maximum scale for zero-point quantization
            zeros_min_scale: Minimum scale for zero-point quantization
            
        Returns:
            Tuple of quantized scales and zero-points
        """
        shape = scale_per_bits.size()
        scale_reshaped = scale_per_bits.reshape(self.shape[0], -1).t().contiguous()
        
        with torch.no_grad():
            qscale, qscale_scale, qscale_zero = min_max_quantize(
                scale_reshaped,
                bits=self.double_quant_scale_bits,
                group=self.double_group_size,
                asym=True,
                max_scale=scale_max_scale,
                min_scale=scale_min_scale,
            )
        
        # Apply STE for scale quantization
        quantized_scale = ((qscale - qscale_zero) * qscale_scale).view(scale_reshaped.size())
        scale_reshaped = (quantized_scale - scale_reshaped).detach() + scale_reshaped

        # Process zero-points for asymmetric quantization
        if not self.symmetric:
            zeros_reshaped = zeros_per_bits.reshape(self.shape[0], -1).t().contiguous()
            with torch.no_grad():
                qzero, qzero_scale, qzero_zero = min_max_quantize(
                    zeros_reshaped,
                    bits=self.double_quant_zero_bits,
                    group=self.double_group_size,
                    asym=True,
                    max_scale=zeros_max_scale,
                    min_scale=zeros_min_scale,
                )
            
            # Apply STE for zero-point quantization
            quantized_zero = ((qzero - qzero_zero) * qzero_scale).view(zeros_reshaped.size())
            zeros_reshaped = (quantized_zero - zeros_reshaped).detach() + zeros_reshaped
            zeros_per_bits = zeros_reshaped.t().contiguous().reshape(shape)

        scale_per_bits = scale_reshaped.t().contiguous().reshape(shape)
        
        return scale_per_bits, zeros_per_bits
    ...
    def _fast_dynamic_quantization_and_fake_quant(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Supports both group-wise and channel-wise quantization.
        ...
        for i in range(len(self.bits_column_end_index)):
            ...
            if group_size == -1:
                # Channel-wise quantization: process each row independently
                tensor_reshaped = tensor_per_bits  # Shape: [out_channels, in_features]
            else:
                # Group-wise quantization: reshape into groups
                tensor_reshaped = tensor_per_bits.reshape(-1, group_size)#.contiguous()
            ...
            if self.qat:
                scale_per_bits, zeros_per_bits = self.double_quant(
                    scale_per_bits,
                    zeros_per_bits,
                    self.scale_max_scale_2th[i],
                    self.scale_min_scale_2th[i],
                    self.zeros_max_scale_2th[i],
                    self.zeros_min_scale_2th[i],
                )
    ...
    def _compute_quantization_params(
        ...
        Handles both group-wise and channel-wise quantization based on group_size.
        """
        group_size = self.group_sizes[bit_index]
        
        if self.n_bits[bit_index] > 1:
            # Multi-bit quantization
            if group_size == -1:
                # Channel-wise quantization: compute per-row (per output channel)
                xmin = tensor.amin(dim=-1, keepdim=True)  # Shape: [out_channels, 1]
                xmax = tensor.amax(dim=-1, keepdim=True)  # Shape: [out_channels, 1]
            else:
                # Group-wise quantization: compute per-group
                xmin = tensor.amin(dim=-1, keepdim=True)
                xmax = tensor.amax(dim=-1, keepdim=True)
            ...
            # 1-bit quantization
            if group_size == -1:
                # Channel-wise 1-bit quantization
