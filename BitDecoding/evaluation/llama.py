    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if past_key_value is not None:
            # sin and cos are specific to RoPE models; cache_position needed for the static cache
            cache_kwargs = {"sin": sin, "cos": cos, "cache_position": cache_position}
            key_states, value_states = past_key_value.update(key_states, value_states, self.layer_idx, dim=-2, cache_kwargs=cache_kwargs)
...
class LlamaFlashDecodingAttention(LlamaAttention):
    """
    Llama flash attention module. This module inherits from `LlamaAttention` as the weights of the module stays
    untouched. The only required change would be on the forward pass where it needs to correctly call the public API of
    flash attention and deal with padding tokens in case the input contains any of them.
...
class LlamaBitDecoding(LlamaAttention):
    """
    Llama flash attention module. This module inherits from `LlamaAttention` as the weights of the module stays
    untouched. The only required change would be on the forward pass where it needs to correctly call the public API of
    flash attention and deal with padding tokens in case the input contains any of them.
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if q_len == 1:
            ...
            if self.quant_mode == 'k-channel':
                ...
                k_params = torch.zeros((batch_size, int(seqlen_k_pack // self.group_size), nheads_k, d), dtype=torch.float32, device=device)
            else:
                k_pack   = torch.zeros((batch_size, seqlen_k_pack, nheads_k, int(d // self.pack_nums)),  dtype=torch.uint16, device=device)
                k_params = torch.zeros((batch_size, int(d // self.group_size), nheads_k, seqlen_k_pack), dtype=torch.float32, device=device)

            v_pack   = torch.zeros((batch_size, seqlen_k_pack, nheads_k, int(d // self.pack_nums)),  dtype=torch.uint16, device=device)
            v_params = torch.zeros((batch_size, int(d // self.group_size), nheads_k, seqlen_k_pack), dtype=torch.float32, device=device)
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        self.self_attn = LLAMA_ATTENTION_CLASSES[config.attn_backend](config=config, layer_idx=layer_idx)
        # self.self_attn = LlamaFlashDecodingAttention(config=config, layer_idx=layer_idx)
        # self.self_attn = LlamaFlashAttention2(config=config, layer_idx=layer_idx)
        # self.self_attn = LlamaBitDecoding(config=config, layer_idx=layer_idx)
